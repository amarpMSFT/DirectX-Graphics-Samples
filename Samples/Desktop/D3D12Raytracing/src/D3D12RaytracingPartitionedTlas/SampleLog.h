...
// appends to a log file at %TEMP%\<SAMPLE_LOG_DEFAULT_NAME>.  The file path is
// overridable at runtime via the SAMPLE_LOG env var.  The file is truncated on
// first use within a process.
//
// SAMPLE_LOG_DEFAULT_NAME is the per-sample log filename baked at compile
// time -- typically set in the .vcxproj as a preprocessor define, e.g.:
//     SAMPLE_LOG_DEFAULT_NAME=L"D3D12RaytracingPartitionedTlas.log"
// If unset, defaults to "Sample.log" so the header still compiles standalone.
//
...
#ifndef SAMPLE_LOG_DEFAULT_NAME
#define SAMPLE_LOG_DEFAULT_NAME L"Sample.log"
#endif
